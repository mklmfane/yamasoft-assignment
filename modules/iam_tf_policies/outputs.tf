output "tf_backend_rw_policy_arn" {
  ...
  value = length(trimspace(var.existing_backend_rw_policy_arn)) > 0 ? trimspace(var.existing_backend_rw_policy_arn) : try(aws_iam_policy.tf_backend_rw[0].arn, "")
}

output "tf_vpc_apply_policy_arn" {
  description = "ARN of the Terraform VPC apply policy (created or existing)."
  value = length(trimspace(var.existing_vpc_apply_policy_arn)) > 0 ? trimspace(var.existing_vpc_apply_policy_arn) : try(aws_iam_policy.tf_vpc_apply[0].arn, "")
}
